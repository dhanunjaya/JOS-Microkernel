...
env_setup_vm(struct Env *e)
{
	...
	// LAB 3: Your code here.
                    pde_t* envPage=KADDR(page2pa(p));
                    p->pp_ref+=1;
                    memset(envPage,0,PGSIZE);
                    e->env_pgdir=envPage;
                    e->env_cr3=page2pa(p);
                    int i=PDX(UTOP); 
                    for(;i<1024;i++)
                      {
                          e->env_pgdir[i]=boot_pgdir[i];
                               // mapping the kernel space.....all wud be identical if we load this page directory in cr3....the va when previous one was pagedir, it will still pointing to the same address as its the same  
                      }
...
segment_alloc(struct Env *e, void *va, size_t len)
{
        ...
        //   'va' and 'len' values that are not page-aligned.
        //   You should round va down, and round (va + len) up.
        uint32_t nPgVirtAddr = 0;
        struct Page *pp = NULL;
        uint32_t vaStart = ROUNDDOWN((uint32_t)va, PGSIZE);
        uint32_t vaEnd = (uint32_t)va + len;
        vaEnd = ROUNDUP(vaEnd, PGSIZE);
        for( nPgVirtAddr = vaStart; nPgVirtAddr < vaEnd; nPgVirtAddr+=PGSIZE )
        {
                pp = NULL;
             cprintf("va= %x len=%d\n",nPgVirtAddr,vaEnd);
                if( 0 > page_alloc(&pp) )
                        return;
                if ( 0 > page_insert(e->env_pgdir, pp, (void *)nPgVirtAddr, PTE_W|PTE_U) )
                        return;
        }
}
*/
     ...
     for (;ph < eph; ph++)
                {
                 ...
                 //cprintf("\nafter segment%x    %x    %x   %d\n",va,binary,va+ph->p_offset,sizeof(binary));
                         //memmove((uint8_t *)va,binary + ph->p_offset,ph->p_filesz);
    //                       cprintf("after setting\n");
                         //memset(((uint8_t *)va)+ph->p_filesz,0,ph->p_memsz-ph->p_filesz);
      //                   cprintf("after setting\n");
...
load_icode(struct Env *e, uint8_t *binary, size_t size)
{
        ...
        // Save the old value of the cr register
        uint32_t cr3 = rcr3();
        // Load the the new value with
        lcr3(e->env_cr3);
        struct Proghdr *ph = NULL, *eph = NULL;
        // load the start address of the program header array
        ...
        // Save the previous value of cr3 after writing the data in the env address space
        lcr3(cr3);
          ...
          if(curenv!=e)
              {
                 ...
                 lcr3(e->env_cr3);
                       ...
                       env_pop_tf(&e->env_tf);	// LAB 3: Your code here.
