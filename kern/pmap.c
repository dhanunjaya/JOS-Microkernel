...
i386_vm_init(void)
{
	...
	// Your code goes here:
      ...
      //    pages=boot_freemem;
          // cprintf("npage=%d  bytes=== %d",npage,npage*((sizeof(struct Page))));
      //   
...
//panic("i386_vm_init: This function is not finished\n");	//////////////////////////////////////////////////////////////////////
	...
	// Current mapping: VA KERNBASE+x => PNENV*sizeof(struct Env)A x.
...
// these if necessary. 
//
// *pp_store -- is set to point to the Page struct of the newly allocatedpage_alloc
// page
//
// RETURNS 
//   0 -- on success
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
        ...
        if(user_mem_check_addr < ULIM)
        {
                ...
                {
                        pte_t *pte = pgdir_walk(env->env_pgdir,(void *)user_mem_check_addr,0);
                        if((pte == NULL)||(!(*pte & (perm|PTE_P))))
                                return -E_FAULT;
                        else
                        {
                                //cprintf("user mem check,env:%08x,pte:%08x\n",env->env_id,*pte);
